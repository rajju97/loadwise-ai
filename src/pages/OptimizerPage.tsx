...
export function OptimizerPage() {
  ...
  const [vehicle, setVehicle] = useState<Vehicle>(demoVehicles[2])
  const [items, setItems] = useState<LoadItem[]>(demoItems)
  const [objective, setObjective] = useState<Objective>('balanced_utilization')
  ...
  function addItem(event: FormEvent<HTMLFormElement>) {
    ...
    const next: LoadItem = {
      id: crypto.randomUUID(), name: String(data.get('name')), sku: String(data.get('sku') || ''), quantity: Number(data.get('quantity')),
    ...
    <AppShell title="Load optimizer" subtitle="Build a balanced, space-efficient plan with real cargo constraints.">
      ...
      <div className="optimizer-layout">
        <div className="optimizer-left">
          <section className="panel config-panel"><div className="panel-head"><div><span className="section-index">01</span><h3>Select vehicle</h3></div><a className="text-button" href="/app/vehicles"><Plus size={15} /> Add vehicle</a></div>
